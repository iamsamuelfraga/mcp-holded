...
const BASE_URL = 'https://api.holded.com/api/crm/v1';
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        if (!response.ok && this.retryableStatusCodes.has(response.status)) {
          ...
          lastError = new Error(`Holded CRM API error (${response.status}): ${errorText}`);
        ...
        if (!response.ok) {
          const errorText = await response.text();
          throw new Error(`Holded CRM API error (${response.status}): ${errorText}`);
        }
        ...
        if (error instanceof Error) {
          ...
          if (error.message.includes('Holded CRM API error')) {
